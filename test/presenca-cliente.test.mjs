...
test('privado: fechar a conversa e a lista tira o texto do DOM (o diagnóstico leva o DOM inteiro)', () => {
  const c = novoCliente();
...
test('privado: o diagnóstico da presença leva CONTAGENS — nenhum nome, texto ou token', async () => {
  const c = novoCliente();
  c.P.Presenca.online = [pessoa(CAF, 'cafanha_nome_unico', 0, 0)];
  c.P.Presenca.conversas = [conversa('9', 'outro_nome_unico', 1, 2, { texto: 'mensagem_privada_unica' })];
  c.P.Presenca.chat = { token: 'TOKEN_UNICO', base: 'https://x/', chave: 'CHAVE_UNICA', expiraEm: Date.now() + 1e8 };
  const diag = JSON.stringify(c.P.presencaDiag());
  for (const s of ['cafanha_nome_unico', 'outro_nome_unico', 'mensagem_privada_unica', 'TOKEN_UNICO', 'CHAVE_UNICA', CAF]) {
    assert.ok(!diag.includes(s), `o diagnóstico levou ${s}`);
  }
