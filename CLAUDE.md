...
- **`route.fulfill` NÃO popula o cache HTTP do navegador**, então stub não consegue medir NADA que dependa dele — e o cache do navegador é o mecanismo INTEIRO da foto offline. MEDIDO com controle: a imagem interceptada carrega na 1ª vez e QUEBRA offline na 2ª; o mesmo byte vindo de um servidor de verdade volta do cache sem tocar a rede. É por isso que a seção da ESTRADA no `tools/smoke-offline.mjs` serve a foto de um arquivo REAL do próprio servidor (mesma origem, logo permitida pela CSP, com `Cache-Control` de verdade). Irmão do `fulfill` não passar pela rede, que a fila de saída já tinha pago.
