...
**Smoke de browser (`npm run test:browser`, roda no CI):** `tools/smoke-browser.mjs` renderiza o card em 5 aparelhos × 4 idiomas × 4 tipos de pedido e MEDE — rolagem dupla, alvo de toque < 44px, estouro horizontal, área rolável sem nome, caixa longa com teto fixo, português vazando fora do pt. Mora em `tools/` e **não** em `test/` de propósito: o `node --test` varre o diretório `test/` inteiro e o smoke precisa de browser — dentro de `test/` ele entrava no `npm test` e quebrava a promessa de suíte com zero dependência (já aconteceu). No CI o Playwright entra com `npm i --no-save playwright@1.49.1` + `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1` (o postinstall baixaria chromium+firefox+webkit, ~400MB) e o chromium vem DEPOIS, do PRÓPRIO Playwright (`npx playwright install chromium`, ~30s). **A versão é FIXA e NÃO é a do sandbox** (aqui o global é 1.56.1), e essa divergência já custou um diagnóstico inteiro: um defeito que só aparece em 1.49 foi "refutado" três vezes por não reproduzir aqui (ver o `EvalError`, na seção da fila de saída). **Antes de escrever "não reproduz localmente", confira a versão.** Reproduzir a do CI hoje é isto, e não mais que isto:

```bash
PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1 npm i --no-save playwright@1.49.1
PLAYWRIGHT_BROWSERS_PATH=/tmp/pw49-browsers npx playwright install chromium
PLAYWRIGHT_BROWSERS_PATH=/tmp/pw49-browsers npm run test:offline
```
