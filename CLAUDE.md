...
- **A visibilidade LIGA SOZINHA, em silêncio** (decisão do owner). O `perfil` devolve `visivelNoWme` (de graça, está no `/Session`); desligada e nunca vista ligada → a próxima ação leva `visivel: true`. **Vista ligada e agora desligada = a pessoa se escondeu FORA da app** (no WME ou noutro aparelho): conta como desligar o "Ver quem está na fila", com o mesmo carimbo da anistia de 9 dias — sem isso a app "desesconderia" a pessoa a cada abertura. O "já vista" é `preferences.presencaWmeVisto` (estrito `=== true`, some no "Sair"). **Desligar o interruptor manda `visivel: false` NA HORA** (a única requisição própria da fase 2, e só no gesto); religar pede ligar na próxima ação, sem esperar o freio.
