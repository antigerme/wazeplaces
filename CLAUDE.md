...
- ⚠️ **RODE `date -u +%Y%m%d` ANTES DE BUMPAR.** O serial é `data + revisão`: em dia novo o certo é **zerar a revisão** (`2026072403` → `2026072501`), não seguir incrementando (`…04`). Já errei isso: dois deploys do dia 25 saíram carimbados como 24, e o rodapé passou a mentir sobre a data — que é justamente a única coisa que este esquema promete entregar. Não dá pra automatizar sem falso positivo (um PR aberto de madrugada e mergeado depois da meia-noite quebraria a checagem), então é disciplina: confira a data, não confie na memória nem no serial anterior.
