...
### A presença e o chat DO WME — a troca da sala própria (fase 1: só servidor)

Decidido pelo owner em 2026-09-23, com o relatório medido na mesa: a app vai usar a presença ("editores online") e o chat que o WME já tem, e depois aposentar a sala, o WebRTC e o TURN. **Fase 1 = só servidor**: `server/wme-grpc.mjs` (o fio), as rotas `presenca-waze` e `chat` no core, e nenhuma tela chamando. As fases seguintes (presença por card, chat, avatares no mapa, remoção) **mudam tela e pedem mockup antes**. Os números completos, cada um com procedência, estão em `docs/waze-api.md` §4.1; aqui fica o que decide código:
...
- **A app NUNCA desliga a visibilidade sozinha.** É a mesma chave do perfil do WME: desligar aqui esconderia a pessoa lá sem ela saber. Para sair da lista basta parar.
...
| `presenca-waze` | gRPC `MapEditorWebServer/{updateOnlineEditor,listOnlineEditors}` em `/<região>-Descartes/grpc/` | Presença DO WME (fase 1, nenhuma tela chama ainda). Move a pessoa (`posicao` e/ou `visivel`, com o `userId` dela) e lista quem está online numa `caixa`, na MESMA ida. O Waze recusa id que não é o do cookie. Ver a seção de presença |
