...
**DOIS DEFEITOS DESTE RECURSO CHEGARAM AOS TESTADORES, e a causa dos dois é a mesma: NENHUM teste ligava o service worker** (47 contextos com `serviceWorkers: 'block'` no smoke de layout, ZERO com `'allow'`). Isso agora tem casa própria — `tools/smoke-offline.mjs`, `npm run test:offline`, no CI — e um guard META em `test/offline.test.mjs` que reprova se a cobertura sumir, se o script sair do `package.json` ou se o CI parar de rodá-la. Os DOIS defeitos foram sabotados de volta e o smoke novo reprova nos dois.
