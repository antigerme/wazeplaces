...
- **Tempo verbal e AUTORIA**: a ação que você oferece tem que concordar com o que já aconteceu, e com quem fez.
  “Desfazer” pressupõe que foi o EDITOR quem fez — oferecê-lo para algo que a APP fez sozinha está errado mesmo
...
│   ├── presenca.js          # Presença + conversa (fase 3): a lista de quem usa a app e o chat,
...
│   ├── marca-app.mjs        # A MARCA de quem usa a app, gravada nos últimos dígitos da posição da
...
- **A folha que abre é SÓ LEITURA.** O pedido não está na fila de quem recebe; ✕ ↑ ✓ ali abririam um caminho de ação novo sobre algo que a pessoa não tem. Quem quiser agir abre o ↗ — a mesma saída que a app já dá pra tudo que ela não decide.
- **A ação NÃO mora no card** (avaliado e recusado pelo owner): o botão no card obrigaria a app a perguntar "pra quem?", abrindo um segundo modo na folha de presença, e apareceria/sumiria conforme quem estivesse online — layout do card mudando por motivo alheio ao card.
...
Decidido pelo owner em 2026-09-23, com o relatório medido na mesa: a app vai usar a presença ("editores online") e o chat que o WME já tem, e depois aposentar a sala, o WebRTC e o TURN. **Fase 1 = só servidor**: `server/wme-grpc.mjs` (o fio), as rotas `presenca-waze` e `chat` no core, e nenhuma tela chamando. **Fase 2 = quem usa a app aparece no mapa do WME** (sem mudar tela, ver abaixo). As fases seguintes (lista e chat na app, remoção da sala) **mudam tela e pedem mockup antes**. Os números completos, cada um com procedência, estão em `docs/waze-api.md` §4.1; aqui fica o que decide código:

**O MODELO, do owner, e ele não é "a app é o WME":** a infra do Waze serve a presença e o chat, mas a app trata SÓ o que é entre usuários da app. Quem está no WME vê os usuários da app no mapa e pode mandar mensagem pra eles, pelo WME; na app, a lista de presença mostra só quem usa a app, no país do filtro, e as conversas são só as que começaram na app (fase 3). Eu entendi errado duas vezes antes de ele fechar ("a app é só uma casca do WME" NÃO quer dizer mostrar todo mundo do WME) — releia antes de desenhar a fase 3.

- **A presença expira ~15 min depois da ÚLTIMA atualização, de qualquer tipo.** O WME só escreve no `moveend` do mapa, então WME aberto e PARADO some igual a fechado (medido: 14,5 min presente, 16,6 fora). Na app a posição vai de carona nas ações (com freio de 30 s), e quem para some sozinho. Manter online quem está parado exigiria um pulso a cada 14 min, que é o polling que o free tier proíbe.
- **A MARCA de quem está na app mora na POSIÇÃO** (`server/marca-app.mjs`, fonte única de quem escreve e de quem vai ler). A lista do Waze não diz quem está na app, e a posição é o único campo que a pessoa escreve. MEDIDO com as duas contas: o Waze devolve os inteiros ×1e6 EXATOS que recebeu (3 de 3, um de controle com dígitos redondos). A marca é `lat µ° mod 100 = 47` e `lon µ° mod 1000 = countryId` — o país vai NA marca, sem tabela de fronteira. Custo: o bonequinho anda no máximo 50 µ° (~5,6 m) na lat e 500 µ° (~55 m) na lon; chance de posição qualquer do WME cair na marca E no país: 1 em 100 mil, e o custo disso é baixo porque quem está no WME já pode falar com quem usa a app. **Resto MATEMÁTICO, não do valor absoluto**: pelo absoluto as posições marcadas ficam com um buraco em volta do zero e o deslocamento DOBRAVA perto de Greenwich — o teste dos extremos pegou na primeira versão. Única exceção ao teto: a linha de data. **TODA posição que a app escreve sai marcada**, inclusive pela rota `presenca-waze` (que por isso exige `pais`): posição sem marca tiraria a pessoa da lista dos outros usuários da app.
- **O eco confere a marca de graça.** O `updateOnlineEditor` devolve o registro com a posição gravada; a carona compara e responde `marca: true|false|null` (`null` = sem posição pra conferir, que NÃO é "perdeu"). O dia em que o Waze arredondar, a lista da app esvazia calada — por isso `marca: false` vira `presencaWme.marcaPerdida` no diário e alerta no resumo do diagnóstico.
...
- **Do lado da app, a posição é montada DENTRO do executor**, na hora do envio (`presencaWmeDaAcao`), e é a do card NA TELA naquele instante (o próximo, depois do gesto — é onde a pessoa está olhando); sem card na tela, a do pedido. **FREIO de 30 s** entre escritas: o WME de quem olha não redesenha sozinho (medido na fase 1), e dobrar as chamadas ao Waze no ritmo do swipe é a assinatura que faz um WAF marcar a conta. **Nunca pela fila de saída, lote, recusa automática ou descarga**: a posição seria velha, e nenhum item guarda posição. Nunca lança: qualquer erro ali vira "sem posição" e a ação segue.
- **A visibilidade LIGA SOZINHA, em silêncio, e quem decide é SÓ o interruptor da app** (decisões do owner, 2026-09-23 e 2026-09-24). O `perfil` devolve `visivelNoWme` (de graça, está no `/Session`); com o "Ver quem está no app" ligado, invisível no WME → a próxima ação leva `visivel: true`, SEMPRE — por padrão a pessoa aparece, como a comunidade e os champs decidiram. **Até v2026.09.24-01 valia o oposto**: invisível DEPOIS de a app já a ter visto ligada (`preferences.presencaWmeVisto`) contava como "se escondeu fora da app" e desligava o interruptor por 9 dias. Saiu, com o preço à mostra: é a MESMA chave nos dois lugares, então quem se esconde pelo WME e segue triando com o interruptor ligado volta a aparecer, no WME também, na ação seguinte — pra ficar invisível usando a app, o caminho é o interruptor. **E o medo que trouxe a pergunta não se confirmou**: abrir o WME NÃO desliga ninguém. MEDIDO com a cafanha: carregar o WME não escreveu nada, arrastar o mapa mandou UMA escrita com a máscara só em `location`, e a visibilidade seguiu ligada. O "invisível" que se vê no WME é o valor inicial de quem nunca ligou (a app liga na 1ª ação) ou a saída da LISTA depois de 15 min parado (a chave segue ligada). O marcador antigo não é mais lido (o próximo `savePreferences` o descarta), e `test/presenca-wme.test.mjs` reprova se ele voltar ao código. **Desligar o interruptor manda `visivel: false` NA HORA** (a única requisição própria da fase 2, e só no gesto); religar pede ligar na próxima ação, sem esperar o freio.
- **A app NUNCA desliga a visibilidade POR CONTA PRÓPRIA.** É a mesma chave do perfil do WME: desligar esconderia a pessoa lá sem ela saber. Só o gesto dela desliga (o interruptor), e sair da lista também acontece parando (15 min).
...
**Fase 3 = a lista e a conversa na app** (v2026.09.24-01, o desenho A dos mockups aprovados; rota `presenca-app`, `chat` com `abrir`):
- **A lista é a do WME filtrada pela marca e pelo país** (`filtrarOnlineDaApp`): caixa do MUNDO (48 editores, 2,9 KB medidos), quem pede não vem, invisível não vem. Duas partes: "Triando agora" (do mais perto pro mais longe, com "a X km daqui" do card NA TELA até a posição marcada da pessoa — Haversine, km inteiros crus) e "Conversas" (as 5 mais recentes de quem NÃO está na app, e toda com não lida, que número que não se zera é o gotcha #66). O subtítulo usa o nome do país que o FILTRO mostra (o do Waze, em inglês): o mockup dizia "Brasil", e consistência vence tradução. Não há nível nas conversas de quem está fora: a lista de conversas não traz rank.
- **A conversa é da app pela MARCA ou por ser CONHECIDA** (`filtrarConversasDaApp`). A marca (`contexto.app = 'wazeplaces'`) é posta pelo SERVIDOR em todo `enviar` — o cliente não a tira nem troca. Sozinha ela não basta: a resposta dada pelo WME vem sem marca e tiraria a conversa da lista no meio. Por isso o aparelho guarda as conversas que já viu como da app (≤ 50) e as manda de carona (`conhecidos`). Mensagem de quem só usa o WME: nada na app (decisão do owner) — mas é confirmada, senão volta a cada reconexão.
- **A lista NÃO anda sozinha** (free tier): vem de carona nas ações (`presencaApp` na resposta, junto da `presenca` da fase 2, com o mesmo teto de espera e o mesmo freio de 30 s) e custa UM pedido ao abrir a app, ao abrir a lista e ao voltar do segundo plano (no máximo um por minuto). Quem abriu antes só vê quem chegou depois numa dessas horas — o smoke abria na ordem errada e "reprovou" a app por isso; hoje ele tem esse CONTROLE.
...
- **O pedido mandado**: o texto que vai pro Waze leva a pergunta, o 📍 com nome e tipo e o link LONGO do ↗ (`linkWmeDoPedido`). O link curto (só `env` e `venues`) foi MEDIDO no WME de verdade: abre a 7.755 km e não seleciona nada. O cartão vai no contexto (`card`) com a pergunta curta (`legenda`, 280 caracteres — a pergunta inteira estouraria o teto de 6 KB do servidor); na app, a pergunta volta do TEXTO (`presencaLegendaDoTexto`). O tipo do pedido passa pelo `rotuloDeEnum`, como no card: o cartão vem de outro aparelho, que pode ser de uma versão com tipo que esta não conhece.
- **Custo, medido no smoke**: abrir a app 1 pedido (lista + token), abrir a lista 1, abrir a conversa 1 (histórico e "lida" juntos), mandar 1; mensagem chegando 0; confirmar 0.
...
| `presenca-waze` | gRPC `MapEditorWebServer/{updateOnlineEditor,listOnlineEditors}` em `/<região>-Descartes/grpc/` | Presença DO WME. Move a pessoa (`posicao` e/ou `visivel`, com o `userId` dela) e lista quem está online numa `caixa`, na MESMA ida. Posição exige `pais` e sai MARCADA (`marca-app.mjs`). O Waze recusa id que não é o do cookie. Na fase 2 a app só a chama no gesto de desligar; mover e ligar vão de carona. Ver a seção de presença |
| `presenca-app` | gRPC `listOnlineEditors` (caixa do mundo) + `MessagingHistory/ListConversations` (e `GetMessagingProvider` com `token: true`) | A lista e as conversas DA APP (fase 3): quem tem a marca da app e o país do filtro (`filtrarOnlineDaApp`), e as conversas com a marca na última mensagem ou `conhecidos` do aparelho (`filtrarConversasDaApp`). Exige `pais` e `userId`. Com `token: true` + `instalacao`, devolve o token do tempo real. Leva a confirmação de carona (`confirmar` + `instalacao` → `confirmados`). Recusada pelo `diag-api` (lê a prévia das conversas e devolve credencial). A MESMA lista volta de carona nas ações, em `presencaApp` |
| `chat` | gRPC `com.waze.wmp.{Messaging,MessagingHistory}` em `/<região>-wmp/` | Chat DO WME (fases 1 e 3). `acao`: `token` (fluxo de tempo real, exige `instalacao`), `naoLidas`, `conversas`, `mensagens`, `abrir` (histórico + "lida" numa ida; página antiga não marca), `perfis`, `enviar` (o SERVIDOR põe a marca da app no contexto), `recibo`, `lida`, `confirmar`. Toda ação aceita a confirmação de carona (`confirmar` + `instalacao`); pedido recusado não confirma nada. Recusada pelo `diag-api` (lê conversa privada e devolve credencial) |
...
**O filtro de permissão NÃO substitui o portão do login, e a intuição erra aqui.** MEDIDO abrindo o portão num servidor local e logando uma conta **L1 não-AM** de verdade: ela recebe **498** pedidos do Brasil, contra **188** da conta L6+AM do owner — ou seja o filtro passa MAIS pra quem tem menos rank, porque o `permissions` vem do Waze por venue e no WME rank governa NÍVEL DE BLOQUEIO, e a maioria dos locais é lock 1. Consequência: o portão (`isStaff || rank >= 1 && isAreaManager`, no servidor) é a única coisa que separa uma conta recém-criada de 498 pedidos reais com poder de rejeitar e marcar lido — não "o Waze já protege". (O que o Waze protege é o que a pessoa poderia fazer À MÃO no WME; o que a app adiciona é velocidade.) **Foi baixado de L3+AM para L2+AM em 2026-09-09, decisão do owner com esse número na mesa.** O que sustenta a decisão é o **AM**, não o rank: área é concedida pela comunidade, então quem tem uma foi avalizado independentemente do nível. O que continua fechado em L6+AM é todo o destrutivo (excluir/aprovar foto, renomear, recusa automática); o que abre com L2 é rejeitar e marcar como lido, na velocidade do swipe. **A medição que falta**, se um dia isso for reavaliado: a dos 498 foi feita com conta **L1 não-AM** — ninguém mediu o que uma L2+AM de verdade recebe.
...
**O defeito que isto conserta estava MEDIDO no código de antes**, e o caminho era: o placar sobe e é GRAVADO, o card sai da tela, 3s de janela, envia, falha, duas retentativas (1,5s e 3,5s) — e aí o último ramo do `handleActionResult` **reverte o placar e descarta a ação**. O editor vê o número voltar atrás uns 5 segundos depois de já ter seguido em frente, com o pedido intacto no Waze. A app não tinha **nenhuma** noção de estar offline, nem uma chave no dicionário.
...
- **A espera do fim do esvaziamento mora em `tools/esperar-saida.mjs`, fonte única — e ela foi extraída porque a lição não pega quando mora num arquivo que o outro teste não importa.** O comentário dela já listava três armadilhas do `page.waitForFunction` (polla por rAF, que não dispara em página de segundo plano; com `polling: <ms>` usa o timer DA PÁGINA, que é justamente o que o runner estrangula; e o `.catch(() => {})` engole a rejeição, virando "não esperei" sem rastro) — e eu repeti as três, palavra por palavra, escrevendo a seção da ESTRADA no smoke do offline. Com uma QUARTA por cima: a assinatura é `(pageFunction, arg, options)`, então `waitForFunction(fn, { timeout, polling })` passa as opções como ARGUMENTO e nada do que você escreveu vale. Poll pelo lado do NODE, por sinal POSITIVO, e IMPRIMA o motivo. **E o smoke do offline MEDE a superfície de erro da app** (`pageerror` mais o `unhandledrejection` que ela mesma captura), então instrumento que injeta maquinaria na página vira ruído indistinguível de defeito: o CI acusou um `EvalError: Refused to evaluate a string as JavaScript` que a app **não pode** produzir — ela não tem `eval` nenhum. **DIAGNOSTICADO em 2026-09-22, e o que faltava era eu estar medindo na MESMA versão.** Duas hipóteses tinham sido "refutadas" (o `polling` numérico dispara eval; a espera derrubada deixa rejeição) — as duas testadas no Playwright **1.56.1 do sandbox**, enquanto o CI fixava o **1.49.1**. "Não reproduz aqui" não valia nada.
    ...
    O poller do rAF avalia **string dentro da página**, e a CSP da app — que com razão não tem `unsafe-eval` — o barra. Vira promessa rejeitada, o `unhandledrejection` da PRÓPRIA app a registra com `console.error`, e o smoke acusa "erro de JS" e "violação de CSP": **o mesmo evento contado duas vezes**, que é exatamente o par que reprovou o CI.
...
- **A janela servida (`offlineJanelaServida`) só avança quando a varredura TERMINA** — e é GRAVADA na mesma base da fila (`offlineGravarJanela`) no mesmo tique. Virá-la antes faz o card pedir um sufixo não aquecido com a cópia boa parada no cache, a um sufixo de distância; não gravá-la faz a app REABERTA sem rede nascer com a janela nula e pedir a foto crua, que a varredura nunca guarda.
...
- **A app reaberta sem rede não achava foto nenhuma.** A janela do sufixo morava só em memória: renascida (o Android encerra app em segundo plano), a app pedia a foto crua. Hoje ela é gravada junto da fila e volta antes do primeiro card.
...
```js
{
  ...
  autoPendente,           // janela da recusa automática. Slot PRÓPRIO, FORA do pendingAction de propósito: o
                          //   acoesTravadas() congela os botões quando VOCÊ age e a app espera confirmação, e
                          //   congelar por uma decisão que a app tomou sozinha seria pior que o problema.
...
**O gargalo do `localStorage` aqui é a GRAVAÇÃO POR SWIPE, não os 5 MB.** `recordHistory` grava a cada ação confirmada e `setItem` é SÍNCRONO: ele trava a thread principal, que é exatamente o valor central da app. Medido no Chromium, um mapa `nome → [contagem, datas]`:
...
Um quadro a 60fps são 8,3 ms; os 5 MB só apertariam lá pelos 130.000, muito depois de já estar engasgando. Referência útil: o histórico por DIA custa 139 KB e 3,5 ms **depois de 10 anos** de uso — a app já carrega uma estrutura dessa ordem e ninguém percebeu.
...
**Três decisões de tela que saíram de MEDIÇÃO** (mockups na app real, Pixel e Fold, 2 temas, pt e fr — e as três davam tudo verde na medição que a app já tinha):
...
**E o ponto LEVA ao que destravou** (v2026.09.20-04, o owner olhando a app: *"é estranho a pessoa desbloquear uma conquista, aparecer a bolinha e ao clicar não levar direto para o treco de interesse"*). Ele tinha razão e a incoerência era literal: o Desfazer abre a aba certa **e** destaca a linha; o ponto abria em **Filtros** e largava a pessoa diante de 16 células. Os dois ANÚNCIOS seguem diferentes de propósito (banner × ponto) — o que não podia diferir é o que acontece quando a pessoa aceita o convite. Cinco coisas que não são gosto:
...
- **A ROLAGEM não entra no portão de `reduced-motion`; o pulso entra.** Quem aponta é a rolagem — tirá-la de quem prefere menos movimento é não levar a pessoa a lugar nenhum. `scrollIntoView` é instantâneo, não é animação. E é `center` e não o `nearest` da dica: ali o gesto é da pessoa e rolar o mínimo evita mexer a tela sob o dedo; aqui a APP é que está apontando.
- **O pulso é âmbar e só `box-shadow`.** Âmbar porque é a cor do conceito "novo" em toda a app (ponto, contorno, etiqueta são `#d97706`) — o que o separa do contorno estático embaixo dele é o MOVIMENTO, não o tom. Só sombra porque célula de grade que muda de tamanho empurra as vizinhas.
...
- **Duas sentinelas, as duas de invariante que a app garante.** `fotoEscondidaComAviso`: o aviso
...
- **Rede em tudo**: `rede.caiu`/`rede.voltou` no `dfato` (e `naAbertura` quando a app já abre sem
...
- **O tique no aparelho ao confirmar é de 8ms** (`VIBRACAO_COMMIT_MS` em `js/swipe.js`; era 12, baixado pelo owner em 2026-09-20 depois de comparar 12/10/8 no celular dele). É o sinal **mais frequente da app** — um por pedido tratado, e a fila real tem centenas —, então o que passa despercebido num toque isolado vira presença constante no ritmo do swipe. **Só existe no Android**: o Safari do iPhone não implementa a Vibration API, e o `if (navigator.vibrate)` é o que faz isso degradar calado. **Mexer nisso não se prevê pela tabela** — muitos motores têm tempo MÍNIMO de acionamento e achatam qualquer pulso curto, então meça no APARELHO e ÀS CEGAS (saber o número enfeita a percepção; o Chromium do sandbox não tem motor nenhum). A outra `navigator.vibrate` da app — o "peguei" do FAB do modo dev — **não é esta** e segue em 12ms de propósito: marca outro conceito e acontece uma vez a cada muitas sessões.
- **O card NOVO não nasce com efeito NENHUM** (decisão do owner em duas rodadas, 2026-09-20, olhando a app rodar: primeiro *"o próximo card não pode nascer com efeito nenhum"*, e, depois de ver o fade rodando na app, *"pode tirar o fade"*). Era UMA animação só — a classe `.card-enter` — e ela arrastava atrás de si um defeito e um remendo; confundir os três é fácil, então: **animação** = o fade; **defeito** = o card de baixo aparecendo ATRAVÉS do da frente enquanto ele está translúcido; **remendo** = a regra que escondia o de fundo durante o fade. Tirando a animação, os dois outros somem por construção, e foi por isso que saíram juntos.
  ...
  - **O defeito nasceu do PAR fade+pilha, não do fade.** MEDIDO no pixel da foto (o card que entra laranja, o de baixo roxo): **3 quadros** devolviam a foto do card de BAIXO e dava pra ler os dois nomes sobrepostos por ~150ms — era isso que lia como "a app travou pra carregar o de baixo". Antes da pilha o card entrava translúcido sobre o fundo da página e não havia o que atravessar. **Nenhuma captura estática pegaria**: só aparece em movimento, e o smoke media o arraste, não a transição depois de soltar.
  - **Quem reintroduzir efeito na entrada reintroduz o par inteiro** — e aí voltam também as duas redes que o remendo exigia: `animationend` **não dispara** com `animation: none` (o caso de quem usa `prefers-reduced-motion`), então era preciso a regra declarativa no bloco de reduced-motion **e** um teto por `setTimeout`, porque classe pendurada deixava o card de fundo escondido pra sempre e a pilha sumia da app sem erro nenhum. `test/pilha.test.mjs` cobra a ausência dos três (fonte e `app.css` gerado), e o smoke mede na tela — com CONTRAPROVA que injeta o fade e o esconderijo de volta, senão "opacidade sempre 1" e "nunca escondeu" passariam com a medição cega (gotcha #28).
...
- **Em português, "app" é MASCULINO** (decisão do owner, 2026-09-24: *"normalmente falamos 'no app' e não 'na app'"*): o app, no app, do app, pelo app, este app, um app. A concordância vai junto: "o app fica aberto", "Ele rejeita…". "Aplicação" é o termo de Portugal; na tela é "app" (ou "aplicativo"). Vale pra toda string pt, pro README, pro CHANGELOG e pras mensagens ao owner. `test/i18n.test.mjs` reprova artigo ou preposição feminina colada em "app", no dicionário pt e no texto visível do HTML. A concordância que vem depois do "app" ele não enxerga; essa se confere ao escrever. **Este arquivo, os docs internos e os comentários do código ainda dizem "a app"**: é texto interno, que não foi convertido, e a forma daqui não se copia pra UI nem pras mensagens.
...
65. **Modal: duas armadilhas que expulsam ou escondem, as duas em silêncio.** (a) **Fechar um e abrir outro no MESMO quadro** dessincroniza o voltar: o `closeModal` AGENDA um `history.back()`, o `openModal` seguinte EMPILHA, e o back pendente come a entrada nova — sobra profundidade sem entrada real, e o próximo fechamento sai da app. `history.length` e o contador parecem certos o tempo todo. Use `openModal` sozinho: ele já esconde os outros. (b) **Modal aninhado em modal** some junto com o pai: o botão existe no DOM e não é clicável, sem nada no console. Os dois travados em `test/layout.test.mjs`. O (a) mordeu o produto uma vez ("Conectar outro aparelho") e os meus scripts de teste TRÊS.
