...
### A presença e o chat DO WME — a troca da sala própria (fases 1 e 2)
...
- **O tempo real vai do navegador DIRETO ao Google** (`instantmessaging-pa.googleapis.com`, CORS aberto a qualquer origem), com o token de 24 h que a rota `chat`/`token` devolve. Custo zero pro nosso `/api`. A fase do chat terá de pôr esse host no `connect-src` das TRÊS cópias da CSP.
- **O fluxo só entrega com a conexão aberta**: reconectar não reentrega nada. Ao voltar de segundo plano, a app consulta as não lidas (1 requisição) — nunca polling.
...
| `presenca` | `/row-Descartes/app/Session` (só pra emitir o crachá) | Devolve `cracha` (nome/rank/AM assinados + a sala já resolvida) e `ice`. Aplica o MESMO gate do login |
...
| `chat` | gRPC `com.waze.wmp.{Messaging,MessagingHistory}` em `/<região>-wmp/` | Chat DO WME (fase 1). `acao`: `token` (fluxo de tempo real, exige `instalacao`), `naoLidas`, `conversas`, `mensagens`, `perfis`, `enviar`, `recibo`, `lida`, `confirmar`. Recusada pelo `diag-api` (lê conversa privada e devolve credencial) |
