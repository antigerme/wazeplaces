...
- **Só passa `^[A-Z][A-Z0-9_]*$`.** `network` traz marca própria (`Belib'`, `ChargeGuru`) e `locationInVenue` traz frase — humanizar ali corromperia o nome (gotcha #39). A mesma regex barra booleano; havia uma linha explícita pra ele e era **código morto**, provado por sabotagem que passava limpa.
