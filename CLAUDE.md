...
- **UM indicador para os dois estados**, e o giro só aparece quando está MESMO saindo (`inFlightActions > 0`). "Esperando" com spinner seria a app fingindo trabalho justamente no estado em que não há rede pra trabalhar. **Sem toast por ação**: 150 pedidos numa sombra dariam 150 interrupções; quem presta contas é o "N esperando envio", que some sozinho. **Ele COBRE número do placar, e isso foi avaliado e MANTIDO pelo owner** (2026-09-21, olhando mockups na tela real): medido, o balão flutuante tapa **100%** da tinta do `RESTAM` no iPhone e **95%** do `REJEITADOS` no Galaxy Fold, nos dois temas — o que contraria o `.nao-cobrir` do gotcha #26 e o comentário do `#avisoSessao` (*"indicador de estado permanente ali cobriria o card"*). As duas alternativas foram desenhadas e medidas (linha sob o placar, como o aviso de sessão, e chip no mesmo lugar): as duas zeram a cobertura e custam 20–24px de card no Fold, 0 no iPhone. O owner escolheu ficar com o balão. **Não re-proponha sem dado novo** — a tentação vai voltar, porque a medição acusa e a regra existe; ela já foi paga.
