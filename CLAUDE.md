...
**O filtro de permissão NÃO substitui o portão do login, e a intuição erra aqui.** MEDIDO abrindo o portão num servidor local e logando uma conta **L1 não-AM** de verdade: ela recebe **498** pedidos do Brasil, contra **188** da conta L6+AM do owner — ou seja o filtro passa MAIS pra quem tem menos rank, porque o `permissions` vem do Waze por venue e no WME rank governa NÍVEL DE BLOQUEIO, e a maioria dos locais é lock 1. Consequência: o portão (`isStaff || rank >= 2 && isAreaManager`, no servidor) é a única coisa que separa uma conta recém-criada de 498 pedidos reais com poder de rejeitar e marcar lido. Se um dia se discutir baixá-lo, é decisão de PRODUTO com esse número na mesa — não "o Waze já protege". (O que o Waze protege é o que a pessoa poderia fazer À MÃO no WME; o que a app adiciona é velocidade.)
