...
- **O card NOVO não nasce com efeito de MOVIMENTO — só um fade de 140ms** (decisão do owner, 2026-09-20, olhando a app rodar: *"o próximo card não pode nascer com efeito nenhum"*). A entrada era uma MOLA de 280ms (`cubic-bezier(0.34, 1.56, 0.64, 1)`) que deslizava 26px, escalava de 94% a 100% e fazia fade. **O que cada peça custava, MEDIDO no DOM quadro a quadro** — e o owner descreveu o sintoma antes de eu medir (*"parece que o card saiu de um tamanho menor para maior"*): a escala fazia o card **crescer 24px de largura** (339 → 363px), e o `1.56` da curva o levava a **363,1px contra 361px finais**, com o topo passando 5px do lugar antes de voltar; somadas, **233ms de card mexendo** na frente de quem já está decidindo o próximo. Só com o fade: 0px de movimento, 0px de variação de tamanho, e a classe some em 140ms.
  - **E o fade só pôde ficar porque veio com um conserto junto.** Ele deixa o card da frente TRANSLÚCIDO, e desde a pilha existe um card inteiro atrás — MEDIDO no pixel da foto (o que entra laranja, o de baixo roxo): **3 quadros devolviam a foto do card de BAIXO** e dava pra ler os dois nomes sobrepostos por ~150ms. Era isso que lia como "a app travou pra carregar o de baixo", e **nenhuma captura estática pegaria**: só aparece em movimento, e o smoke media o arraste, não a transição depois de soltar. Conserto: `.card-enter ~ .card-fundo { visibility: hidden }`, o que devolve o comportamento de antes da pilha (durante o fade aparece o FUNDO DA PÁGINA, não outro pedido).
  - **A classe `.card-enter` tem que sair de qualquer jeito**, e isso agora é segurança e não arrumação: enquanto ela estiver no card, o de fundo fica escondido. `animationend` **não dispara** com `animation: none` — o caso de quem usa `prefers-reduced-motion` —, então há as DUAS redes: a regra `visibility: visible` dentro do bloco de reduced-motion, e um `setTimeout(tirarEntrada, ENTRADA_TETO_MS)`. O teto é **maior** que a animação de propósito e **não** é a duração copiada: número duplicado em dois arquivos é como eles divergem (a lição do `UNDO_WINDOW_MS` no doc).
